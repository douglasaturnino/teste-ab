...
def probabilidade_normal(distruibuicao_normal, media, variancia):
    return norm.pdf(
        distruibuicao_normal, loc=media, scale=1.25 * np.sqrt(variancia)
    )
...
def get_chart_data():
    ...
    data = data.pivot(
        index="day", columns="group", values=["click", "visit"]
    ).fillna(0)
    data.columns = [
        "click_control",
        "click_treatment",
    ...
    bayesian[colunas] = pd.DataFrame(
        np.row_stack(
            np.vectorize(bayesian_inference, otypes=["O"])(
                data["acc_clicks_A"],
                data["acc_clicks_B"],
                data["acc_visits_A"],
                data["acc_visits_B"],
...
def load_data():
    url = os.getenv("WEB_URL", "http://127.0.0.1:5000")
    url = url + "/dados"
    r = requests.get(url)
